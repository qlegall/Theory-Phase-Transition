...
\begin{flalign*}
  &\mathbb{E}\left(\prod_{i=1}^{p} \xi_{z_i} \right) = \mathbb{E}\left(\mathbb{E}\left(\prod_{i=1}^{p} \xi_{z_i} \vert \Lambda \right)\right) &\\ \label{eq2} &= \mathbb{E}\left(\mathbb{E}\left(\mathbbm{1}\lbrace R(Q_n(nz_1)) < n \rbrace \mathbbm{1} \lbrace X^{\lambda} \cap Q_n(nz_1) = \emptyset \rbrace \ldots \mathbbm{1}\lbrace R(Q_n(nz_p)) < n \rbrace \mathbbm{1} \lbrace X^{\lambda} \cap Q_n(nz_p) = \emptyset \rbrace \vert \Lambda \right) \right) &
\end{flalign*}

Now, what I'd like to do in  is getting  those indicators of the form $\mathbbm{1} \lbrace R(Q_n(nz_i) <n \rbrace$ out of the conditional expectation, so as to get:

\begin{align}
    \eqref{eq2} &= \mathbb{E}\left(\mathbbm{1}\lbrace R(Q_n(nz_1)) < n \rbrace \ldots \mathbbm{1}\lbrace R(Q_n(nz_p)) < n \rbrace \mathbb{E}\left(\mathbbm{1} \lbrace X^{\lambda} \cap Q_n(nz_1) = \emptyset \rbrace \ldots \mathbbm{1} \lbrace X^{\lambda} \cap Q_n(nz_p) = \emptyset \rbrace \vert \Lambda \right) \right) \\ \label{eq4} &= \mathbb{E}\left(\mathbbm{1}\lbrace R(Q_n(nz_1)) < n \rbrace \ldots \mathbbm{1}\lbrace R(Q_n(nz_p)) < n \rbrace \prod_{i=1}^{p}\mathbb{E}\left(\mathbbm{1} \lbrace X^{\lambda} \cap Q_n(nz_i) = \emptyset \rbrace \vert \Lambda \right) \right)
\end{align}
where Poisson independence property has been used to obtain \eqref{eq4}. Now, setting, $f(\Lambda_{Q_n(x)}) \coloneqq \mathbb{E}\left(\mathbbm{1} \lbrace X^{\lambda} \cap Q_n(x) = \emptyset \rbrace \vert \Lambda \right)$, a deterministic measurable function of $\Lambda_{Q_n(x)}$ we are left with:
\begin{align}
    \eqref{eq4} &= \mathbb{E}\left( \prod_{i=1}^{p} f(\Lambda_{Q_n(nz_i)}) \mathbbm{1}\lbrace R(Q_n(nz_i)) <n \rbrace \right) \label{eq5}
\end{align}
Now, the set $\varphi \coloneqq \lbrace nz_1,\ldots,nz_p \rbrace \subset \mathbb{R}^{d}$ is finite and satisfies: $\forall x \in \varphi, \text{dist}(x,\phi \setminus \lbrace x \rbrace ) > 3n$, so we can apply the definition  \cite[Definition 2.3]{hirsch_continuum_2017} of stabilization to get that the random variables appearing in~\eqref{eq5} are independent. Hence $\eqref{eq5}=\prod_{i=1}^{p} \mathbb{E}\left( f(\Lambda_{Q_n(nz_i)})\mathbbm{1}\lbrace R(Q_n(nz_i)) <n \rbrace \right)$ \\
Therefore, we finally get:
\begin{align}
    \mathbb{E}\left(\prod_{i=1}^{p} \xi_{z_i} \right) &= \prod_{i=1}^{p} \mathbb{E}\left(\mathbb{E}\left( \mathbbm{1} \lbrace X^{\lambda} \cap Q_n(nz_i) \neq \emptyset \rbrace \vert \Lambda \right) \mathbbm{1} \lbrace R(Q_n(nz_i)) <n \rbrace \right) \label{eq7}
\end{align}
Putting back the indicators of the form $\mathbbm{1}\lbrace R(Q_n(nz_i)) <n \rbrace$ inside of the conditional expectation in~\eqref{eq7}, we are left with the expectation of a conditional expectation and finally get the desired identity:
\begin{align}
    \mathbb{E}\left(\prod_{i=1}^{p} \xi_{z_i} \right) &= \prod_{i=1}^{p} \mathbb{E}\left( \mathbbm{1} \lbrace X^{\lambda} \cap Q_n(nz_i) \neq \emptyset \rbrace   \mathbbm{1} \lbrace R(Q_n(nz_i)) <n \rbrace \right) \\ &\eqqcolon \prod_{i=1}^{p}\mathbb{E}(\xi_{z_i})
\end{align}
and so we're done.
\end{Proof}
